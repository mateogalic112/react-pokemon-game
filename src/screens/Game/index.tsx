...
const invalidGoDown = (position: number): boolean => position >= GO_DOWN_BOUNDARY;
const invalidGoRight = (position: number): boolean => position % GRID_COLUMNS === GO_RIGHT_BOUNDARY;
const invalidGoUp = (position: number): boolean => position < GO_UP_BOUNDARY;
const invalidGoLeft = (position: number): boolean => position % GRID_COLUMNS === GO_LEFT_BOUNDARY;
...
const Game = () => {
  ...
  const foePokemonsWithPositions: Record<number, Pokemon> = foePokemons.reduce((acc, foe, idx) => {
    return {
      ...acc,
    ...
    switch (event.code) {
      ...
      case "ArrowRight":
        ...
        if (checkEncounterWithFoe(goRight(playerPosition))) {
          return battleWithFoe(foePokemonsWithPositions[goRight(playerPosition)]);
        }
        socket.emit("update_player_position", {
          trainerName: trainer.name,
      ...
      >
        {Array.from(Array(600).keys()).map((_, idx) => (
          <GridItem
            ...
            alignItems="center"
          >
            {!!onlinePlayers?.find((player) => player.position === idx) ? <Player /> : null}
